...
void hash_first (struct hash_iterator *i, struct hash *h) {
  ...
  i->hash = h;
  i->elem = NULL;
  /* 모든 버킷을 순회하면서 첫 번째 원소가 있는 버킷을 찾음 */
  for (b = 0; b < h->bucket_cnt; b++) {
    if (!list_empty(&h->buckets[b])) {
      i->bucket = &h->buckets[b];
      i->elem = list_elem_to_hash_elem(list_begin(i->bucket));
      break;
    }
  }
  /* 만약 모든 버킷이 비어 있다면, i->elem은 NULL로 남음 */
  if (b == h->bucket_cnt)
    i->elem = NULL;
}
...
struct hash* create_Hash() {
  struct hash* new = (struct hash *)calloc(1, sizeof(struct hash));
  hash_init(new, no_change_hash_int, really_less, NULL);
  return new;
}
// create_Hash에서 hash_int 실제로 변화 없이 사용하기 위해서 따로 함수 설정정
unsigned no_change_hash_int(const struct hash_elem *h, void *aux) {
  return hash_int(h->real);
}
// 해쉬 elem 생성
struct hash_elem* make_Hash_elem(int real_data) {
  struct hash_item *new = malloc(sizeof(struct hash_item));  // 올바른 크기 사용
  if (!new) return NULL;
  new->real = real_data;
  return &new->elem;
}
...
void Hash_square(struct hash_elem* h, void* aux) {
  struct hash_item* item = hash_item_entry(h);
  item->real = item->real * item->real;
}

// 3배하기
void Hash_triple(struct hash_elem* h, void* aux) {
  struct hash_item* item = hash_item_entry(h);
  item->real = item->real * item->real * item->real;
  
}
// 해시요소 삭제 free시키기기
void remove_Hash_elem(struct hash_elem* h, void* aux) {
  free(h);
  return ;
}
// 2,3배
void apply_Hash(struct hash* h, char* com) {
  if (!strcmp(com, "square")) { // square 만나면 2배배
    hash_apply(h, Hash_square);
  } else if (!strcmp(com, "triple")) { // triple 만나면 3배
